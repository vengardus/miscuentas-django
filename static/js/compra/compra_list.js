...
let controller_name = '/compra_controller/'
...
class FormApp extends Form {
    ...
    get_filter(filter) {
        return g_aDataTable.filter((table) => 
                table.comercio_desc.toLowerCase().indexOf(filter.toLowerCase()) != -1
                || table.tarjeta_desc.toLowerCase().indexOf(filter.toLowerCase()) != -1
                || table.detalle.toLowerCase().indexOf(filter.toLowerCase()) != -1
...
function get_data_columns(item) {
    return [
        { data: item.fecha_compra, class: '' },
        { data: item.tarjeta_desc, class: '' },
        { data: item.comercio_desc, class: '' },
        { data: item.monto, class: '' },
        { data: item.cuotas, class: '' },
        { data: (item.is_fuera_cierre)? 'Si':'No', class: '' },
    ]
}
