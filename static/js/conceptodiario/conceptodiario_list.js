...
let controller_name = '/conceptodiario_controller/'
...
class FormApp extends Form {
    ...
    get_filter(filter) {
        return g_aDataTable.filter((table) => 
                ...
                || table.rubro_diario_desc.toLowerCase().indexOf(filter.toLowerCase()) != -1
                || table.tipo_movimiento_desc.toLowerCase().indexOf(filter.toLowerCase()) != -1
            );
    }
...
function get_data_columns(item) {
    return [
        ...
        { data: item.rubro_diario_desc, class: '' },
        { data: is_type_screen(g_min_size_screen.desktop) ? 
                    item.tipo_movimiento_desc :
                    item.tipo_movimiento_desc.substring(0,3),
          class: '' },
    ]
}
