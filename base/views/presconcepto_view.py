...
from base.forms.presconcepto_form import PresconceptoForm


@login_required(login_url='login')
def presconcepto_list(request):
    oListView = ListView(Presconcepto)
    oListView.template_container = PARAMS.TemplateContainerMain
    oListView.template = PARAMS.Template.presconcepto_list
    oListView.actions['action_new'] = reverse('presconcepto_form', args=['new', 0])
    oListView.set_context()
    return render(request, oListView.template, context=oListView.context)


@login_required(login_url='login')
def presconcepto_form(request, mode, id):
    url_return = 'presconcepto_list'
    oBModel = BPresconcepto()
    ...
    oTemplate.template = PARAMS.Template.presconcepto_form
