...
from base.business.bcomercio import BComercio
from base.models import Comercio
from base.forms.comercio_form import ComercioForm


@login_required(login_url='login')
def comercio_list(request):
    oListView = ListView(Comercio)
    oListView.template_container = PARAMS.TemplateContainerMain
    oListView.template = PARAMS.Template.comercio_list
    oListView.actions['action_new'] = reverse('comercio_form', args=['new', 0])
    oListView.set_context()
    return render(request, oListView.template, context=oListView.context)


@login_required(login_url='login')
def comercio_form(request, mode, id):
    url_return = 'comercio_list'
    oBModel = BComercio()
    ...
    if request.method == 'POST':
        ...
        if mode == 'new':
            ...
            form = ComercioForm(request.POST, instance=oTO)
        ...
        if mode == 'new':
            ...
            form = ComercioForm(instance=oTO)

    oTemplate = Template(Comercio)
    oTemplate.template_container = PARAMS.TemplateContainerMain
    oTemplate.template = PARAMS.Template.comercio_form
