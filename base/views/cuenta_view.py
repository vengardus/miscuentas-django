...
from base.forms.cuenta_form import CuentaForm


@login_required(login_url='login')
def cuenta_list(request):
    oListView = ListView(Cuenta)
    oListView.template_container = PARAMS.TemplateContainerMain
    oListView.template = PARAMS.Template.cuenta_list
    oListView.actions['action_new'] = reverse('cuenta_form', args=['new', 0])
    oListView.set_context()

    oBCuenta = BCuenta()
    oBCuenta.get_all(license_id=request.user.license_id)
    oListView.data['resumen'] = oBCuenta.get_resumen()
...
def cuenta_form(request, mode, id):
    ...
    oTemplate.template = PARAMS.Template.cuenta_form
