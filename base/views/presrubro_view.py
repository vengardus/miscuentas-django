...
from base.forms.presrubro_form import PresrubroForm


@login_required(login_url='login')
def presrubro_list(request):
    oListView = ListView(Presrubro)
    oListView.template_container = PARAMS.TemplateContainerMain
    oListView.template = PARAMS.Template.presrubro_list
    oListView.actions['action_new'] = reverse('presrubro_form', args=['new', 0])
    oListView.set_context()
    return render(request, oListView.template, context=oListView.context)


@login_required(login_url='login')
def presrubro_form(request, mode, id):
    url_return = 'presrubro_list'
    oBModel = BPresrubro()
    ...
    if request.method == 'POST':
        ...
        if mode == 'new':
            form = PresrubroForm(request.POST)
        else:
            form = PresrubroForm(request.POST, instance=oTO)
        ...
        if mode == 'new':
            ...
            form = PresrubroForm(instance=oTO)

    oTemplate = Template(Presrubro)
    oTemplate.template_container = PARAMS.TemplateContainerMain
    oTemplate.template = PARAMS.Template.presrubro_form
