...
from base.business.bcambiomoneda import BCambioMoneda
from base.models import CambioMoneda
from base.forms.cambiomoneda_form import CambioMonedaForm


@login_required(login_url='login')
def cambiomoneda_list(request):
    oListView = ListView(CambioMoneda)
    oListView.template_container = PARAMS.TemplateContainerMain
    oListView.template = PARAMS.Template.cambiomoneda_list
    oListView.actions['action_new'] = reverse('cambiomoneda_form', args=['new', 0])
    oListView.set_context()
    return render(request, oListView.template, context=oListView.context)


@login_required(login_url='login')
def cambiomoneda_form(request, mode, id):
    url_return = 'cambiomoneda_list'
    oBModel = BCambioMoneda()
    ...
    if request.method == 'POST':
        ...
        if mode == 'new':
            form = CambioMonedaForm(request, request.POST)
        else:
            form = CambioMonedaForm(request, request.POST, instance=oTO)
        ...
        if mode == 'new':
            form = CambioMonedaForm(request)
        else:
            form = CambioMonedaForm(request, instance=oTO)
    

    oTemplate = Template(CambioMoneda)
    oTemplate.template_container = PARAMS.TemplateContainerMain
    oTemplate.template = PARAMS.Template.cambiomoneda_form
